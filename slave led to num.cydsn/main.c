...
int main()
{
    ...
    for (;;)
    {
        ...
        if (0u != (I2CS_I2CSlaveStatus() & I2CS_I2C_SSTAT_RD_CMPLT))
        {
            ...
            if(i2cWriteBuffer[5] == (i2cWriteBuffer[0]^i2cWriteBuffer[1]^i2cWriteBuffer[2]^i2cWriteBuffer[3]^i2cWriteBuffer[4])){
            
            for(int i=0; i < 6; i++){
                if(i!=5){
                i2cReadBuffer[i] = i2cWriteBuffer[i]+5;
                /*if(i2cReadBuffer[i] > 30){
                    for(int j=0;i<5;j++){
                        i2cReadBuffer[j] = i+1;
                    }
                }*/
                    ...
                    i2cReadBuffer[i] = i2cReadBuffer[0]^i2cReadBuffer[1]^i2cReadBuffer[2]^i2cReadBuffer[3]^i2cReadBuffer[4];
                }
